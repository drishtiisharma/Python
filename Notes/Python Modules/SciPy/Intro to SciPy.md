...
│   ├── norm.pdf()     → normal PDF
│   ├── norm.cdf()     → normal CDF
│   └── norm.rvs()     → random samples
│
├── scipy.optimize
│   └── minimize_scalar() → minimize function
│
├── scipy.integrate
...
U, S, Vt = linalg.svd(A)
# SVD takes one matrix and decomposes it into three matrices so that its structure can be analyzed and many mathematical/data problems can be handled more easily.
