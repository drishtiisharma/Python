...
- A built-in Python class that manages a pool of worker threads.
- Part of `concurrent.futures` module.
- Allows us to run multiple tasks concurrently using threads.

Explaination: 
`ThreadPoolExecutor` is a high-level tool in Python's `concurrent.futures` module that makes it easier to **create and manage multiple threads**. Instead of manually creating each `Thread`, starting it, and managing it with `.join()`, you create a pool of worker threads and submit tasks to it. The executor automatically assigns submitted tasks to available threads and manages their execution. It is especially useful when you have **many independent tasks**, such as downloading files, making API requests, reading files, or performing other I/O-bound operations. For example, if you have 10 tasks, you can create `ThreadPoolExecutor(max_workers=5)`, and Python will use up to 5 threads to execute those tasks concurrently.

**Benefits**
- No manual thread management needed
- threads are reused, not created/destroyed each time
- Auto-cleanup with context manager - `with`

**When to Use**
- I/O-bound tasks (network requests, file operations, database queries)
- Tasks that spend time waiting rather than computing
- When you need to run many similar tasks in parallel
...
Based on the Programs: [[sequential1.py]], [[sequential2.py]], [[concurrent1.py]], [[concurrent2.py]]

| Program          | Execution                         | Time shown  | What it demonstrates                                   |     |
| ---------------- | --------------------------------- | ----------- | ------------------------------------------------------ | --- |
| `sequential1.py` | Sequential                        | ~10 sec     | Tasks run one after another                            |     |
| `sequential2.py` | Sequential using `.run()`         | ~10 sec     | `.run()` does **not** create new threads               |     |
| `concurrent1.py` | Concurrent, **without `.join()`** | ~0.0004 sec | Main thread doesn't wait for the threads               |     |
| `concurrent2.py` | Concurrent, **with `.join()`**    | ~5 sec      | Threads run concurrently and main waits for completion |     |
